...
END_DATE = '2025-05-31'
TOKEN = 'eyJraWQiOiJRYjZZNEVhSVNseXJoTzdFaVFPeFJjbE0xaTdDTWZuRXQ2VXBaUGhUa21JPSIsImFsZyI6IlJTMjU2In0..Z2KjYfJQ5_15pMLb0lJv1r-VqbEbnmqLBH5NnAiCHfibfLqlxpAEznjJ-Q6ovqQkY9FuYqEJvn7t4DdxaiG2FK3MiawmMwd2rNQk0tt2KO_zpMzhT6vjEHufWuPobHUKUu7eNu71iNrfCmwm4A4D5K6rYlUqKAt3DxpHXWQMNA4cSYi3MXOx10cJ902F8GOpX1gX8PjOvG_RD-DTGXgnKJEm1jcW2v9q4huBx23hyJXr3f9aI_hrNd5CJBtF3FtN6FOS9k6ksrJJCieCvriW68MuFay5xZRrjShzPVzIoCaAK1oNlZ689YAD6wK5MdULjHoJTus4ofAUUxtQDwo8Fw'
...
def process_bulk_projects(project_codes: List[str], start_date: str, end_date: str, aggregation: str, 
    ...
    """
    Process multiple projects and generate a consolidated CSV report with totals
    """
    try:
        ...
        with open(consolidated_csv_filename, 'w', newline='') as consolidated_file:
            ...
            for project_id in project_codes:
                ...
                try:
                    ...
                    if not project_details:
                        ...
                        # Create a blank row for all headers
                        fallback_row = dict.fromkeys(headers, '')

                        # Populate basic known info
                        fallback_row['Project Code'] = project_id
                        fallback_row['Project Name'] = project_name
                        fallback_row['State'] = project_state
                        fallback_row['COD'] = cod_date
                        fallback_row['Size (kW)'] = ''
                        fallback_row['Start Date (YYYY-MM-DD)'] = start_date
                        fallback_row['End Date (YYYY-MM-DD)'] = end_date
                        fallback_row['Av PPA Price ($/kWh)'] = ppa_rate
            ...
            export_project_summary(successful_projects, failed_projects)
...
# Helper function to safely extract numeric values from string
def safe_extract_number(value_str: str) -> float:
    """
    Safely extract a numeric value from a string, handling formatting like commas and percent signs.
...
def generate_project_data(project_id: str, start_date: str, end_date: str, 
    ...
    try:
        ...
        # Extract forecast generation and irradiance
        # forecast_generation = 0
        # monthly_irradiance = {}
        # try:
        #     sites = project_details.get('project', {}).get('sites', {})
        #     systems = next(iter(sites.values()), {}).get('systems', {})
        #     for system in systems.values():
        #         #forecast_generation = system.get('forecast', {}).get('acEnergy', {}).get('monthlyACEnergy', {}).get(month_name, 0)
        #         monthly_irradiance = system.get('forecast', {}).get('irradiance', {}).get('monthlyIrradiance', {})
        #         break
        ...
        try:
            ...
            logger.error(f"Expected generation error: {e}")
        ...
        # Insolation
        #irradiance_hours = expected_gen_data.get('projects', {}).get(project_id, {}).get('irradianceHours', 0)
        #actual_insolation = irradiance_hours / 1000
        actual_insolation = calculate_actual_insolation(expected_gen_data, project_details, project_id)
        logger.info(f"✅ Actual insolation: {actual_insolation:.2f} kWh/m2")
...
def process_csv_files_in_folder(folder_path: str, start_date: str, end_date: str, 
                                aggregation: str, debugger, token: str) -> List[str]:
    """
    Process all CSV files in a folder containing project codes
    
    Args:
        folder_path (str): Path to the folder containing CSV files
        start_date (str): Start date in YYYY-MM-DD format
        end_date (str): End date in YYYY-MM-DD format
        aggregation (str): Aggregation period (FiveMinute, FifteenMinute, ThirtyMinute, Hour, Day)
        debugger: The debugger instance
        token (str): Authentication token
        
    Returns:
        List[str]: List of paths to generated consolidated reports
    """
    ...
    # Create a folder for output consolidated reports in the Reports folder
    reports_folder = create_reports_folder()
    output_folder = os.path.join(reports_folder, f"consolidated_reports_{start_date}_{end_date}")
    os.makedirs(output_folder, exist_ok=True)
    ...
    for csv_file in csv_files:
        ...
        try:
            # Read project codes from the CSV file
            project_codes = read_project_codes_from_csv(csv_file)
            
            if not project_codes:
                logger.warning(f"No project codes found in {csv_file}. Skipping...")
                print(f"No project codes found in {csv_file}. Skipping...")
                continue
            
            # Extract base filename for the consolidated report
            base_filename = os.path.basename(csv_file)
            base_filename = os.path.splitext(base_filename)[0]
            consolidated_filename = f"{base_filename}_{start_date}_{end_date}_consolidated_report.csv"
            consolidated_path = os.path.join(output_folder, consolidated_filename)
            
            # Process the projects from this CSV file with the specified output path
            success = process_bulk_projects_with_output_path(
                project_codes, 
                start_date, 
                end_date, 
                aggregation, 
            ...
            if success and os.path.exists(consolidated_path):
                consolidated_reports.append(consolidated_path)
                print(f"Successfully processed {csv_file} -> {consolidated_path}")
                logger.info(f"Successfully processed {csv_file} -> {consolidated_path}")
            else:
                logger.warning(f"Some projects in {csv_file} failed to process or consolidated report not found")
                print(f"Some projects in {csv_file} failed to process or consolidated report not found")
                
                # Check if the file exists elsewhere (in the original folder)
                original_path = os.path.join(folder_path, consolidated_filename)
                if os.path.exists(original_path):
                    # Copy to the output folder
                    import shutil
                    shutil.copy2(original_path, consolidated_path)
                    consolidated_reports.append(consolidated_path)
                    print(f"Found consolidated report in original folder, copied to: {consolidated_path}")
                    logger.info(f"Found consolidated report in original folder, copied to: {consolidated_path}")
            
        except Exception as e:
            logger.error(f"Error processing CSV file {csv_file}: {e}")
            print(f"Error processing CSV file {csv_file}: {e}")
            debugger.log_error(f"Processing CSV file {csv_file}", e)
    
    # Log the final list of consolidated reports
    logger.info(f"Final consolidated reports: {consolidated_reports}")
    print(f"Final consolidated reports: {consolidated_reports}")
    
    return consolidated_reports


def process_bulk_projects_with_output_path(project_codes: List[str], start_date: str, end_date: str, aggregation: str, 
                        debugger, token: str, csv_input_filename: str = None, output_path: str = None) -> bool:
    """
    Process multiple projects and generate a consolidated CSV report with totals at a specific output path
    
    This is a modified version of process_bulk_projects that allows specifying the output path
    """
    try:
        # Create Reports folder
        reports_folder = create_reports_folder()
        
        # Set the consolidated CSV filename
        if output_path:
            consolidated_csv_filename = output_path
        elif csv_input_filename:
            # Extract just the filename without path and extension
            base_filename = os.path.basename(csv_input_filename)
            base_filename = os.path.splitext(base_filename)[0]
            consolidated_csv_filename = os.path.join(reports_folder, f"{base_filename}_{start_date}_{end_date}_consolidated_report.csv")
        else:
            consolidated_csv_filename = os.path.join(reports_folder, f"consolidated_report_{start_date}_{end_date}.csv")
        
        logger.info(f"Will save consolidated report to: {consolidated_csv_filename}")
        print(f"Will save consolidated report to: {consolidated_csv_filename}")
        
        # Create parent directory if it doesn't exist
        os.makedirs(os.path.dirname(consolidated_csv_filename) if os.path.dirname(consolidated_csv_filename) else '.', exist_ok=True)
...
def main():
    ...
    try:
        # Process multiple projects with consolidated report
        success = process_bulk_projects(PROJECT_CODES, START_DATE, END_DATE, 'Day', debugger, TOKEN)
        if success:
