...
def main():
    ...
    try:
        ...
        if not token:
            logger.error("Failed to obtain authentication token from token.json")
            return
        ...
        # Read project codes from projects.csv
        csv_file_path = 'projects.csv'
        project_codes = read_project_codes_from_csv(csv_file_path)
        
        if not project_codes:
            print("No project codes found in projects.csv. Exiting.")
            return
